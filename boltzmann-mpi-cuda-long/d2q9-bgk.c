               ...
               int** obstacles_ptr, float** av_vels_ptr, int** obstacles_all_ptr)
{
  ...
  *cells_ptr = (t_speed*)malloc(sizeof(t_speed) * sizeLocal);
  ...
  *tmp_cells_ptr = (t_speed*)malloc(sizeof(t_speed) * sizeLocal);
    // printf("1-3 \n");
...
// }
  ...
  *obstacles_ptr = (int *)malloc(sizeof(int) * sizeLocal);
  ...
  if(params->rank == 0){
    ...
    *obstacles_all_ptr = (int*)malloc(sizeof(int) * sizeAll);
...
int dataToDevices(t_param* params, t_speed**  cells_d, t_speed**  tmp_cells_d,
  ...
  // if(params->rank == 0) system("nvidia-smi");
...
void scatter_obstacle(const t_param* params, int *obstacles_all, int *obstacles_local) {
  ...
  //  printf("1-8-2\n");
  //   fflush(stdout);
  //  printf("%d %d %ld \n", rank, params->nyLocal, local_data_count );
  ...
  if (rank == 0) {
    ...
    { // size 是进程数量
            ...
            // printf("rank %d, params->nx: %d, displs: %ld, rcvCounts: %ld\n", i , params->nx, displs[i], rcvCounts[i]);
            //    fflush(stdout);

            //   printf("rank %d, value of ob %d\n", rank, obstacles_all[i]);
            //      fflush(stdout);
    ...
    //         if(obstacles_local[j * params->nx + i] == 1) printf("%d ", obstacles_local[j * params->nx + i]);
    //     }
    //     printf("\n");
    // }
    // exit(0);
...
long long send_up = nyLocal * nx;      // 发送本地的最后一行数据
int send_down = nx;              // 发送本地的第一行数据
int recv_down = 0;               // 接收放到下幽灵单元
long long recv_up = (nyLocal + 1) * nx; // 接收放到上幽灵单元
