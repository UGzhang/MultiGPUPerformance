...
int main(int argc, char* argv[])
{
  ...
  for (int tt = 0; tt < params.maxIters; tt++)
  {
    ...
    // printf("#######rank: %d -- send: %d, rece: %d\n", rank, start, recv_up/local_ncols);
    ...
    // printf("#######rank: %d -- send: %d, rece: %d\n", rank, start+local_nrows - 1, recv_down/local_ncols);
  ...
  else
  {
    ...
    MPI_Send(cells + start * local_ncols, local_nrows * num, MPI_FLOAT, MASTER, tag, MPI_COMM_WORLD);
