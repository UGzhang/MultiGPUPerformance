...
(t_speed* cells, t_speed* tmp_cells, int* obstacles, int ny, int nx, float omega, float w1_flow, float w2_flow)
{
  ...
  ** are in the scratch-space grid */
  ...
  for(long long jj = jdx; jj < ny; jj += gridStrideY)
  {
...
void propagate_rebound_collision(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles){
    ...
    propagate_rebound_collision_kernel<<<blocksPerGrid, threadsPerBlock>>>
          (cells, tmp_cells, obstacles, params.ny, params.nx, params.omega, w1_flow,w2_flow);
